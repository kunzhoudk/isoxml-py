"""CLI for converting polygon prescription shapefiles into ISOXML grids."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Convert polygon .shp prescription map to ISOXML v3/v4 grid type 1 or 2.",
    )
    parser.add_argument(
        "shp_path",
        nargs="?",
...
def options_from_args(args: argparse.Namespace) -> GridFromShpOptions:
...
def main() -> None:
    ...
    if validated_xsd_path is not None:
        print(f"  xsd:        {validated_xsd_path}")
