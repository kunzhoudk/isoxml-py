    ...
    for feature in gdf_grid[[value_field, "geometry"]].itertuples(index=False):
        ...
        for polygon in _iter_polygons(geom):
            ...
            if boundary_mask_mode == "touch":
                ...
                xs = (
                    minx
                    + (np.arange(c0, c1 + 1, dtype=np.float64) + 0.5) * cell_east_size
                )
                ys = (
                    miny
                    + (np.arange(r0, r1 + 1, dtype=np.float64) + 0.5) * cell_north_size
                )
...
def validate_taskdata_xsd(
    ...
    xsd_path = Path(__file__).resolve().parents[3] / "resources" / "xsd" / xsd_filename
...
def convert_grid_from_shp(options: GridFromShpOptions) -> GridFromShpResult:
    ...
    effective_unit, unit_source = _resolve_value_unit(
    ...
    if extent_metric_geom.is_empty or extent_wgs84_geom.is_empty:
        raise ValueError(
            f"Grid extent geometry is empty (mode: {options.grid_extent})."
        )
