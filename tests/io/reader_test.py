...
def test__read_from_xml__when_version_missing__expect_error():
    xml = (
        '<?xml version="1.0" encoding="UTF-8"?><ISO11783_TaskData></ISO11783_TaskData>'
    )
...
def test__read_from_path__when_external_files_ignore__expect_empty_refs():
    path = TEST_RES_DIR / "isoxml/v4/deutz_export"
    task_data, refs = read_from_path(
        path, external_files="ignore", read_bin_files=False
    )
...
def test__read_from_zip__when_archive_has_taskdata_folder__expect_parsed():
    path = TEST_RES_DIR / "isoxml/taskdata_version_converter/small_xml_v3_type_1_auto.zip"
    task_data, refs = read_from_zip(path)
    assert isinstance(task_data, iso3.Iso11783TaskData)
